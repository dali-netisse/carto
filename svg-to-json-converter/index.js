...
// Import modules
import { loadSVG, createXPath, getElementsByLayer, getCalibrationRect, getRectAttributes, getAttribute, getElementType, parsePoints, getLineAttributes } from './lib/parser.js';
import { parseTransform, transformPoint, transformPoints, getNodeTransforms, createCalibrationTransform, createSpecialTransform, multiplyMatrices } from './lib/transformer.js';
import { parsePath, pathToAbsolute, isPolygonPath, pathToPoints } from './lib/pathParser.js';
import { classifyObject, classifyFurniture, parseDeskIds, mapRoomName } from './lib/classifier.js';
...
async function processFile(filename, options) {
  ...
  if (calibrationRect) {
    ...
    if (elemType === 'rect') {
      ...
      console.log('Found path element for calibration, but Perl script only uses rect elements');
      rect = null;
    }
    ...
    if (rect) {
      ...
      if (calibrationElementTransform) {
        ...
        const transformedCorners = transformPoints(corners, calibrationElementTransform);
      ...
      if (targetRect) {
        ...
        console.warn(`${ANSI.reverse}Warning: No calibration data for site ${site}${ANSI.normal}`);
      }
    }
  }
  ...
  // Process background
  const backgroundElements = getElementsByLayer(xpath, doc, ['Contour', 'contour'], 'rect|polygon|path');
  ...
  // Process decor
  const decorElements = getElementsByLayer(xpath, doc, ['Decor', 'Décor', 'decor'], 'rect|polygon|path|line');
  ...
  // Process itineraries
  const itineraryElements = getElementsByLayer(xpath, doc, ['Lignes de couloir', 'lignes de couloir'], 'polyline|path');
  ...
  // Process rooms
  const roomElements = getElementsByLayer(xpath, doc, ['Salles', 'Pièces', 'salles'], 'rect|polygon|path');
  for (const elem of roomElements) {
    ...
    if (classification.class === 'meeting-room' && classification.name) {
      ...
      if (mappedId) {
        ...
        console.warn(`${ANSI.reverse}Warning: No mapping for meeting room: ${classification.name}${ANSI.normal}`);
      }
  ...
  // Process furniture
  const furnitureElements = getElementsByLayer(xpath, doc, ['Mobilier', 'mobilier'], 'rect|polygon|path|circle|line|polyline');
  ...
  for (const elem of furnitureElements) {
    ...
    if (furniture.type === 'desks' || furniture.type === 'meeting') {
      ...
      if (obj.type === 'polyline' && obj.points) {
        // For lines, use the midpoint and calculate direction from endpoints
        ...
        if (points.length === 2) {
          ...
          direction = Math.atan2(points[1][1] - points[0][1], points[1][0] - points[0][0]);
        } else {
        ...
        if (furniture.indicatorX !== undefined && furniture.indicatorY !== undefined) {
      ...
      // Parse desk IDs
      const objects = parseDeskIds(furniture.deskIds, furniture.office, furniture.width, furniture.depth);
      ...
      if (furniture.indicatorX !== undefined) deskObj.indicator_x = furniture.indicatorX;
      if (furniture.indicatorY !== undefined) deskObj.indicator_y = furniture.indicatorY;
      if (furniture.indicatorA !== undefined) deskObj.indicator_a = furniture.indicatorA;
